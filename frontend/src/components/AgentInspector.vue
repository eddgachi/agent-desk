<template>
  <div class="flex flex-col">
    ...
    <div class="flex items-center justify-between px-3 py-2.5 border-b border-gray-800">
      ...
      <button
        v-if="agent"
        class="text-gray-600 hover:text-gray-300 text-xs leading-none"
    ...
    <div v-else class="px-3 py-3 space-y-3 text-xs overflow-y-auto scrollbar-thin">
      ...
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm text-white flex-none shadow-lg"
      ...
      <div class="grid grid-cols-2 gap-1.5 text-xs">
        <InfoRow label="Position" :value="`(${fmt(agent.position_x)}, ${fmt(agent.position_y)})`" mono />
        <InfoRow
          v-if="agent.target_x != null"
          label="Target"
      ...
      <div v-if="currentTask" class="bg-gray-800 rounded-lg p-2.5 space-y-1.5">
        <div class="flex items-start justify-between gap-1">
          ...
          <span class="flex-none px-1.5 py-0.5 rounded text-xs" :class="taskTypeClass(currentTask.type)">{{
            currentTask.type
          }}</span>
        </div>
        ...
        <div v-if="agent.remaining_task_ticks != null" class="flex items-center gap-2">
          <div class="flex-1 bg-gray-700 rounded-full h-1">
            <div class="h-1 rounded-full bg-blue-500 transition-all" :style="{ width: taskProgress + '%' }" />
          </div>
      ...
      <div v-if="agent.memory && agent.memory.length">
        <div class="text-gray-500 text-xs uppercase tracking-widest mb-1.5">Memory</div>
        <div class="space-y-1">
          <div
            v-for="(mem, i) in [...agent.memory].reverse().slice(0, 4)"
            :key="i"
      ...
      <div v-if="agentEvents.length">
        <div class="text-gray-500 text-xs uppercase tracking-widest mb-1.5">Recent Activity</div>
        <div class="space-y-1">
          <div
...
const agent = computed(() => (uiStore.selectedAgentId ? (simStore.agents[uiStore.selectedAgentId] ?? null) : null))
...
const statusClass = computed(() => STATUS_CLASSES[agent.value?.status] ?? 'bg-gray-700 text-gray-300')
...
const agentEvents = computed(() => {
  ...
  return eventsStore.items.filter((e) => e.payload?.agent_id === id || e.payload?.agent_ids?.includes(id)).slice(0, 5)
})
